...
class CTMoCoDataset(Dataset):
    ...
    Args:
        ...
        pair_window_jitter: HU by which each view's window centre and width
            may move (E3); 0 keeps the fixed window.  Needs *pair_overlap*.
    """

    def __init__(self, data_dir, crops_per_volume=20, pair_overlap=None,
                 pair_z_shift=2, pair_scale=None, pair_window_jitter=0):
        if (pair_scale or pair_window_jitter) and not pair_overlap:
            raise ValueError("pair_scale and pair_window_jitter need pair_overlap")
        ...
        self.pair_window_jitter = pair_window_jitter
    ...
    def __getitem__(self, idx):
        ...
        if self.pair_overlap:
            crop_q, crop_k = random_crop_pair(
                ...
                scale=self.pair_scale, window_jitter=self.pair_window_jitter)
        else:
